...
| 🗂️ | **A side panel worth reading** | Cards for the file, the WAVE header, Broadcast Wave metadata, tags, cue markers, loudness, per-channel levels, the live analysis parameters and the cursor. The button at the right end of the transport bar closes it when you want the width for the views, and a window too narrow for both closes it for you until there is room again. |
