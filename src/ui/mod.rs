...
use auriscope::analysis::{
    ColorMap, FileStats, LiveSpectrum, Spectrogram, StftParams, WaveformPyramid, compute_stats,
};
...
impl App {
    ...
    fn loop_region(&self) -> Option<(u64, u64)> {
        ...
        self.selection
            .map(|(a, b)| (a.min(b) as u64, a.max(b) as u64))
            .filter(|(a, b)| b > a)
    }
    ...
    fn handle_shortcuts(&mut self, ctx: &egui::Context) {
        let (space, home, end, left, right, l, f, esc, plus, minus, open, shift) = ctx.input(|i| {
            (
        ...
        if l {
            self.loop_enabled = !self.loop_enabled && self.selection.is_some();
            self.apply_loop();
        }
        if f {
            if self.selection.is_some() && !shift {
    ...
    fn screenshot_hook(&mut self, ctx: &egui::Context) {
        ...
        match armed {
            ...
            None => {}
            Some(t) if t.elapsed() > std::time::Duration::from_millis(1600) => {
